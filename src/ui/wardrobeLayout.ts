...
export function getWardrobeLayout(width: number, height: number): WardrobeLayout {
  ...
  const actionHeight = compact ? Math.min(60, Math.max(46, h * 0.1)) : Math.min(68, Math.max(64, h * 0.1));
  const action = {
  ...
  const detailsHeight = Math.min(108, Math.max(68, h * 0.14));
  const details = {
    x: action.x,
  ...
  const characterHeight = Math.max(1, Math.min(stage.height - 8, preview.height * 0.62));
