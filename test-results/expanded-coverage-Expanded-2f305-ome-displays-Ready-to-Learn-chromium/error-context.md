...
- Name: expanded-coverage.spec.js >> Expanded UI/UX Coverage Tests (32 cases) >> 13. Phonics Home displays Ready to Learn
- Location: tests/expanded-coverage.spec.js:85:3
...
- button "Read Aloud":
  - img
- paragraph: Today's Mission is waiting for you!
- img
...
> 87  |     await expect(page.getByText('Ready to Learn?', { exact: false })).toBeVisible();
  ...
  182 |   });
  183 | 
  184 |   test('32. Empty parent dashboard state renders', async ({ page }) => {
  185 |     // Skip auth by direct injecting auth state
  186 |     await page.addInitScript(() => {
  187 |       window.localStorage.setItem('phonics-game-storage', JSON.stringify({
```