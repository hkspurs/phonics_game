...
- Name: expanded-coverage.spec.js >> Expanded UI/UX Coverage Tests (32 cases) >> 10. Mastery Map renders mastered node color
- Location: tests/expanded-coverage.spec.js:67:3
...
Locator: locator('button').filter({ hasText: '🔢' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: '🔢' }).first()
